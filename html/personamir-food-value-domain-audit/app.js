(() => {
  ...
  const data = window.FOOD_VALUE_DOMAIN_AUDIT_DATA;
  if (!data || !Array.isArray(data.scopes) || !data.sourceDocument) {
    document.body.innerHTML = "<p style='padding:24px'>Food value-domain audit data failed to load.</p>";
    return;
  }
  ...
  function storageKey(name) {
    return `personamir-food-value-domain-audit:${data.sourceSha256}:${slugify(name)}`;
  }
    ...
    elements.scopeNav.innerHTML = data.categoryOrder.map((category) => {
      ...
      return `<section class="nav-group">
        ...
        ${scopes.map((scope) => {
          ...
          const name = scope.id === "global" ? "全局属性" : titleCase(scope.subtype);
    ...
    return `<article class="attribute-card ${hasActivity ? "has-change" : ""}" data-scope-id="${escapeHtml(scope.id)}" data-attribute-key="${escapeHtml(attribute.key)}">
      <header class="attribute-head">
        ...
        <div class="attribute-meta">
          ...
          <span class="badge">${attribute.values.length} 个值</span>
          ${localChangeCount ? `<span class="badge changed">${localChangeCount} 项改动</span>` : ""}
          <button class="icon-button" type="button" data-action="reset-attribute" title="恢复此属性的原始值域" aria-label="恢复此属性的原始值域">↶</button>
        </div>
      </header>
  ...
  function renderHeading() {
    ...
    elements.scopeTitle.textContent = scope.id === "global" ? "全局属性" : `${titleCase(scope.subtype)} 值域`;
  ...
  function exportAudit() {
    ...
    try {
      ...
      const result = {
        audit_schema: "personamir-food-value-domain-audit-result-v1",
      ...
      const filename = `personamir-food-value-domain-audit-${slugify(reviewer)}-${timestamp()}.json`;
  ...
  elements.sourceSummary.textContent = `${data.sourcePoolItemCount.toLocaleString("en-US")} 条商品 · ${data.attributeCount} 个属性作用域 · ${data.valueCount.toLocaleString("en-US")} 个值 · SHA-256 ${data.sourceSha256.slice(0, 12)}`;
